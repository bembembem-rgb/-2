...
function drawBossCutscene(timeLeft, maxTime) {
    ...
    ctx.font = "10px 'Press Start 2P', monospace"; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
    ctx.fillStyle = UI.amber;
    ctx.fillText('УГРОЗА ОБНАРУЖЕНА', padX, 44);

    // Имя босса — единственный дисплейный размер на экране
    ctx.font = "32px 'Press Start 2P', monospace"; ctx.textBaseline = 'middle';
    ctx.fillStyle = UI.txt;
    ctx.fillText(currentBossName, padX, bannerHeight / 2 + 6);

    ctx.font = "12px 'Press Start 2P', monospace"; ctx.textBaseline = 'alphabetic';
...
function drawWorldPass(cam, vx, vy, vw, vh, time) {
    ...
    if (deepVault) {
        ...
        if (deepVault.sealed) {
            ...
            ctx.font = "10px 'Press Start 2P', monospace"; ctx.textAlign = 'center'; ctx.fillStyle = '#74838c';
            ...
            ctx.shadowBlur = 0;
            ctx.font = "10px 'Press Start 2P', monospace"; ctx.textAlign = 'center'; ctx.fillStyle = '#c46dff';
...
function draw() {
    ...
    if (gameState === 'loading') {
        ...
        ctx.font = "20px 'Press Start 2P', monospace"; ctx.fillStyle = UI.cyan; ctx.textAlign = 'center';
