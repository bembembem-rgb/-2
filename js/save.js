...
const ACHIEVEMENTS = {
    first_dive: { title: 'ПЕРВОЕ ПОГРУЖЕНИЕ', desc: 'Дойти до конца первого забега.' },
    warden_down: { title: 'СТРАЖ СНЯТ', desc: 'Свалить ABYSSAL WARDEN.' },
    tide_tamed: { title: 'КАМЕРТОН РАЗБИТ', desc: 'Свалить TIDE CALLER.' },
    leviathan_slain: { title: 'ЛЕВИАФАН РАСПУЩЕН', desc: 'Свалить LEVIATHAN.' },
    reef_cleared: { title: 'РИФ ВСКРЫТ', desc: 'Свалить THORN REEF.' },
    wraith_erased: { title: 'ПРИЗРАК СТЁРТ', desc: 'Свалить VOID WRAITH.' },
    rank_p: { title: 'РАНГ P: ЧИСТАЯ РАБОТА', desc: 'Набрать 2000+ очков за забег.' },
    survivor_5000: { title: 'ВЫЖИВШИЙ: 5000 ОЧКОВ', desc: 'Набрать 5000+ очков за забег.' },
    new_best: { title: 'НОВЫЙ РЕКОРД', desc: 'Побить собственный рекорд.' }
};
const SECRETS = {
    secret_flawless_boss: {
        title: 'ФРАГМЕНТ ПАМЯТИ: ПОСЛЕДНИЙ ИНЖЕНЕР',
        desc: 'Свалить любого босса, не потеряв ни одного HP за бой.',
        reward: 'СКИН «ЧИСТЫЙ СИГНАЛ»: след дайвера становится белым.',
        cutscene: 'Страж не задел тебя ни разу. Пока он рассыпается, помехи на секунду уходят, и в чистом канале проступает чужая строка: если ты это читаешь, протокол ещё можно откатить. Судя по метке, отправили её за два дня до Потопа, и с тех пор её никто не открывал.',
        cosmetic: 'clean_signal'
    },
    secret_speedrunner: {
        title: 'ФРАГМЕНТ ПАМЯТИ: НУЛЕВАЯ ЗАДЕРЖКА',
        desc: 'Дойти до первого босса быстрее чем за 90 секунд.',
        reward: 'СКИН «ПРИЗРАЧНЫЙ ДАЙВЕР»: силуэт становится полупрозрачным.',
        cutscene: 'Ты прошёл мёртвую сеть быстрее, чем защитные протоколы успели проснуться. В окне, которое они не закрыли, лежит карта всех серверов AetherNet разом: девять тысяч узлов, четыре из них ещё горят зелёным. Окно захлопнулось через полсекунды.',
        cosmetic: 'ghost_diver'
    },
    secret_dasher: {
        title: 'ФРАГМЕНТ ПАМЯТИ: ЭХО ДВИЖЕНИЯ',
        desc: 'Набрать 200 рывков за всё время.',
        reward: 'СКИН «ГОЛУБОЙ ШЛЕЙФ»: рывок оставляет длинный след.',
        cutscene: 'После двухсотого рывка сеть узнала твой почерк и подняла из архива файл УЧАСТНИК #0. Там имя, которое ты носил до Потопа, дата найма и пустая графа причины увольнения. Через пару секунд файл закрылся сам, и повторный запрос сеть уже не приняла.',
        cosmetic: 'blue_trail'
    },
    secret_scrapper: {
        title: 'ФРАГМЕНТ ПАМЯТИ: КЛАДБИЩЕ МАШИН',
        desc: 'Разобрать 15 транспортов за один забег.',
        reward: 'СКИН «ЖЕЛЕЗНЫЙ ШТОРМ»: при уроне летят оранжевые искры.',
        cutscene: 'Пятнадцать корпусов оседают на дно. На одном уцелела табличка: ЭВАКУАЦИОННЫЙ ФЛОТ №7, ВЫХОД НЕ НАЙДЕН. Флот ушёл в тот же час, когда вода дошла до твоей серверной, и до орбиты не добрался ни один борт.',
        cosmetic: 'iron_storm'
    },
    secret_pacifist_minute: {
        title: 'ФРАГМЕНТ ПАМЯТИ: ТИХИЙ ЧАС',
        desc: 'Пройти первую минуту забега без единого удара.',
        reward: 'СКИН «ТИХИЕ ВОДЫ»: броня светится ровно, без пульса.',
        cutscene: 'Шестьдесят секунд никто тебя не тронул. В этой паузе гул мёртвой сети проседает почти до нуля, и сквозь него пробивается ровный стук: насосы орбитальной платформы, двести километров вверх. Стук держится секунд десять и тонет обратно.',
